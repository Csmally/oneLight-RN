...
businessHttpsInstance.interceptors.request.use(
  config => {
...
businessHttpsInstance.interceptors.response.use(
  response => {
    // 对响应数据做一些处理
...
(async () => {
  ...
  businessHttpsInstance.defaults.headers['deviceId'] = deviceId;
})();

export default businessHttpsInstance;
